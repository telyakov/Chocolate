!function( $ ) {
    ...
    Datetimepicker.prototype = {
        ...
        place: function(){
            ...
            //сделана затычка - при крайних положениях - не влазит в окно модальный контрол
            var min_offset = 250, win_height = $(window).height(), win_width = $(window).width(), min_diff =330;
            var _top = offset.top + this.height, _left = left;
            if(_top > min_offset){
                var diff = win_height - _top ;
                if(diff < min_diff){
                    _top = win_height - min_diff;
                }
            }

            if(_left > min_offset){

                var diff = win_width - _left ;
                if(diff < min_offset){
                    _left = win_width - min_offset;
                }
            }
