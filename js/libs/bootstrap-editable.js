...
//               if(options.value && options.value.indexOf('.')!=-1)
//                   options.value = options.value.split('.')[0]
////               console.log('test')
//           }
...
//        if(typeof(this.options.unsavedclass) == 'undefined'){
        this.options.unsavedclass = 'editable-unsaved';
//        }

//        console.log($.fn.editableutils.getConfigData(this.$element));
//        console.log($.fn.editable.defaults, options,  $.fn.editableutils.getConfigData(this.$element))
...
//            if(this.options.type=='date'  && this.options.value && this.options.value.indexOf('.'!=-1)){
//                this.options.value =  this.options.value.split('.')[0]
...
//            this.$element.triggerHandler('init', this);
        ...
        toggle: function (closeAll) {
            if (this.container && this.container.tip().is(':visible')) {
                ...
                if (this.input.type === 'select') {
                    ...
                    this._dispatchEvent(event, this.input.$input);
                } else if (this.input.type === 'datetimefield' || this.input.type === 'datefield') {
                    if (this.input.$input !== undefined) {
                        this.input.$input.next('.add-on').trigger('click');
                    }
                }
            }
        },
        _dispatchEvent: function (event, $el) {
            setTimeout(function () {
    ...
    $.fn.editable = function (option) {
        ...
        var count = this.length, _this = this, editableOptions;
        while (count--) {
            var $this = $(_this[count]),
                data = $this.data(datakey),
                options = typeof option === 'object' && option;
            if (!data) {
                $this.data(datakey, (data = new Editable(_this[count], options)));
            }
            if (typeof option === 'string') { //call method
                data[option].apply(data, Array.prototype.slice.call(args, 1));
            }
...
//            y('formatted')
    ...
    var DPGlobal = {
        ...
        //используется в контроле date в карточке
...
(function ($) {
    ...
    $.extend(DateTime.prototype, {
        initPicker: function (options, defaults) {
            ...
            //language
            this.options.datetimepicker.language = this.options.datetimepicker.language || 'ru';
...
(function ($) {
    ...
    $.extend(Constructor.prototype, {
        ...
        sorter: function (items) {
            var beginswith = [], caseSensitive = [], caseInsensitive = [], item, text;
