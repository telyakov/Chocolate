...
class Editable extends CWidget
{
    ...
    */
    ...
    public function registerClientScript()
    {
        ...
        \$context.find('a[rel^=\"{$this->htmlOptions['rel']}\"]')";
        ...
        $name = 'a_'. $this->htmlOptions['rel'];
        $script = 'function ' .$name. '($context){' . $script . '};' .$name. '(); editableCallback.add(' .$name .')';
//        $extendedScript = "alert('test'); $('body').on('hover','.editable',function(e){" .$script .'});' . $script;
        Yii::app()->getClientScript()->registerScript(__CLASS__ . '#' . $this->id, $script);

//        Yii::app()->getClientScript()->registerScript(__CLASS__ . '#' . $this->id, $extendedScript );
//        return $extendedScript;
    ...
    {
        echo '<div class="table-td">'.CHtml::openTag('a', array_merge($this->htmlOptions, array('class'=>'editable')));
        $this->renderText();
